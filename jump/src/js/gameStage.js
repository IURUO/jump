...
export default class GameStage extends THREE.Object3D {
     ...
     */
    addCube() {
        const isToZ = Math.random() > 0.5;
        const distance = Math.random() * 4 + 3;
        const lastPosition = this.children[this.children.length - 1].position;
        const newCube = new THREE.Mesh(new THREE.CubeGeometry(2, 0.5, 2), new THREE.MeshNormalMaterial());
        if (isToZ) {
